...
function Portfolio({ lightMode }) {
  ...
  useEffect(() => {
    fetch("https://createabit-backend.onrender.com/api/v1/service")
      .then((res) => res.json())
      .then((data) => setServics(data.data));
  });
              ...
              <div className="left" id="sticky_item">
                {services?.map((item, index) => (
                  <div
                    ...
                    // data-background={`${lightMode ? "/light/" : "/dark/"}${
                    //   item.image
                    // }`}

                    data-background={`https://createabit-backend.onrender.com/${item.image}`}
                    key={index}
                  ></div>
                ))}
              </div>
                      ...
                      <a className="butn-circle d-flex align-items-center text-center mt-50">
                        <div className="full-width">
                          ...
                          <span className="full-width">
                            <Link
                              href={`/dark/page-service-details/${item.Service_Id}`}
                            >
                              View Details
                            </Link>
                          </span>
